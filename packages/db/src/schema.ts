 ...
 * UUIDs are generated at DB level via gen_random_uuid().
 * RLS policies are in migrations — not here.
 */
...
export const projects = pgTable("projects", {
  ...
  // Five separate locale dimensions (spec requirement)
  uiLocale: varchar("ui_locale", { length: 10 }).notNull().default("en"),
...
export const researchRuns = pgTable("research_runs", {
  ...
  /** Monotonically increasing version within a project */
  ...
  /** Full research plan JSON produced by the planning agent */
  ...
  projectIdIdx: index("research_runs_project_id_idx").on(t.projectId),
}));
...
export const sources = pgTable("sources", {
  ...
  /** Object storage key for raw artifact */
  rawArtifactId: text("raw_artifact_id"),
  /** Object storage key for extracted/parsed artifact */
  extractedArtifactId: text("extracted_artifact_id"),
  /** Is this a demo/seed record? Never display as real evidence. */
  isDemo: boolean("is_demo").notNull().default(false),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (t) => ({
  runIdIdx: index("sources_run_id_idx").on(t.researchRunId),
  contentHashIdx: index("sources_content_hash_idx").on(t.contentHash),
}));
...
export const evidence = pgTable("evidence", {
  ...
  /** JSON {x,y,width,height,page} for visual coordinates */
  coordinates: jsonb("coordinates"),
  capturedAt: timestamp("captured_at", { withTimezone: true }).notNull().defaultNow(),
  /** Extraction confidence 0.0–1.0 */
  confidence: real("confidence").notNull().default(1.0),
  language: varchar("language", { length: 10 }).notNull().default("en"),
  extractionWarnings: text("extraction_warnings").array().notNull().default([]),
  isDemo: boolean("is_demo").notNull().default(false),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (t) => ({
...
export const claims = pgTable("claims", {
  ...
  /**
   * IMPORTANT: status must NEVER be silently upgraded.
   * Changing to "verified" requires human review + evidence.
   */
...
export const dossierCards = pgTable("dossier_cards", {
  ...
  projectIdIdx: index("dossier_cards_project_id_idx").on(t.projectId),
}));
...
export const auditLog = pgTable("audit_log", {
  ...
  diff: jsonb("diff"),
  /** REDACTED before storage — only last octet kept */
